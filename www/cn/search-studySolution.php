...
$dropdown_Name = 'entrylevel';
$dropdown_ID = 'entrylevel';
/***  (Set) Button-Percentage  ***/
$dropdown_Class = 'dropdown_100';

/***  (Set) 1st Value  ***/
$dropdown_valueNil = '请选择「目前学历」';
...
?>
											...
											<div class="gdlr-core-pbf-element" style="display: inline; margin-left: 30px; border: 0px;">
												<button class="gdlr-core-button gdlr-core-button-solid gdlr-core-button-no-border custom-button_noBorder"
													style="display: none; font-weight: bold;" id="btnBack">上 一 步</button>
											</div>
	...
	<script type="text/javascript">
		btnNext_manageEvent();
		/** ====================  << (check) Step-1  >> ==================== **/
		$('select[name="entrylevel"]').change(function () {
			btnNext_manageEvent();
		});
	...
	<script type="text/javascript">
		...
		btn_display();
		$('#entrylevel').change(e => toPg_Next());
		/** ====================  << Next [button]  >> ==================== **/
		$('#btnNext').click(function () {
			switch (stepNum) {
				case 0: { // 「目前学历」
					if ($('#entrylevel').val() == 0) {} else {
						toPg_Next();
					}
					break;
				}
		...
		/** ====================  << AJax  >> ==================== **/
		function getCType() {
			// var tID = $("#entrylevel").children("option:selected").val();
			var solutionID = $('#entrylevel').val();
			$.ajax({
				type: 'GET',
