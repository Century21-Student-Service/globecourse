...
//检测文档正确性
$row = $dosql->GetOne("SELECT * FROM course_hot WHERE id=$id");

if (empty($row)) {
    header("Location: search-course");
    die;
}
$dosql->ExecNoneQuery("UPDATE course_hot SET read_count=read_count+1 WHERE id=$id");
		...
		<div class="kingster-body-wrapper clearfix  kingster-with-frame">
			...
			<div class="kingster-page-wrapper" id="kingster-page-wrapper">
				<div class="gdlr-core-page-builder-body">
					...
					<div class="kingster-page-title-wrap  kingster-style-medium kingster-left-align" style="height:280px;background-image: url('<?php echo $row['image']; ?>');"
						loading="lazy">
						<!-- [Background-Image] -->
						...
						<div class="kingster-page-title-container kingster-container">
							<div class="kingster-page-title-content kingster-item-pdlr ctm-header__content"
								...
								<img class="animated fadeIn ctm-header__logo" src="<?php echo $row['image']; ?>"
										width="" height="" style="border-radius: 10px;" loading="lazy" title="院校 - <?php echo ($row['tag']); ?>" />
										<div class="kingster-page-caption"><h1 class="kingster-page-title" style="padding-top:10px;font-size:38px"><?php echo ($row['tag']); ?></div>
								</h1>
...
$dosql->Execute("select id, title, title_en, tag, tag_en, image from course_hot where id != {$id} order by sort desc, id desc");
while(1){
	$item = $dosql->GetArray();
	if($item == null) break;
...
foreach ($course_spe_list as $item){
        ?>
											<div class="animated fadeIn gdlr-core-pbf-column gdlr-core-column-60" style="text-align: center; margin-bottom: 10px;"
												onclick="parent.location.href='course-hot-info.php?id=<?php echo $item['id']; ?>';">
												<div>
