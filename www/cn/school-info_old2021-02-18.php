...
<head>
	...
	<title><?php echo ($Sch['title']); ?> | 潮流搜索平台</title>
						...
						<div class="kingster-page-title-container kingster-container">
							<div class="kingster-page-title-content kingster-item-pdlr ctm-header__content"
								...
								<a href="school-info.php?id=<?php echo ($Sch['id']); ?>"><img class="animated fadeIn ctm-header__logo"
										src="./../<?php echo ($Sch['picurl']); ?>" width="" height="" style="border-radius: 10px;" loading="lazy" /></a> <!-- [Logo] -->
					...
					<section class="main-content blog-posts course-single ctm-pgSchool__section" style="">
						...
						<div class="container">
							...
							<div class="animated fadeInUp" style="text-align: center; margin-bottom: 50px;">
								...
								<!-- ====================     << Title >>     ==================== -->
								<h1 class="bold ctm-pgSchool__title" style="font-size: 40px; margin-bottom: 0; line-height: normal;"><?php echo ($Sch['title']); ?></h1>
								<!-- (Chinese) -->
								<h1 class="ctm-pgSchool__title-en" style="font-size: 40px; margin-bottom: 16px;"><small><?php echo ($Sch['cname']); ?></small></h1>
								<!-- (English) -->
								...
								<ul class="course-meta review style2 clearfix" style="display: flex; justify-content: center; list-style-type: none;">
									<li class="author">
										<p>所属国家</p>
										<a style="color: #545454 !important;"><strong><?php echo (getCountry($Sch['country'])); ?></strong></a>
									</li>
									<li class="author">
										<p>所属州</p>
										<a style="color: #545454 !important;"><strong><?php echo (getState($Sch['state'])); ?></strong></a>
									</li>
								</ul>
							</div>
...
if ($Sch['schoolType'] > 3) {
    ?>
									<h5 id="tab-cList" class="ctm-tab-top__title <?php echo $onTab_cList; ?>">课程列表</h5>
...
;?>
									...
									<h5 id="tab-hotel" class="ctm-tab-top__title">酒店管理学院</h5>
									<h5 id="tab-eng" class="ctm-tab-top__title">英语课程</h5>
									<?php endif;?>
...
if (!empty($Sch['wpkc'])) {
    ...
    ?>
									...
									<a href="<?php echo $collegeURL; ?>">
										<h5 id="tab-college" class="ctm-tab-top__title">预科/文凭课程</h5>
									</a>
...
?>
									...
									<?php if ($Sch['id'] == 13): ?>
									<h5 id="tab-publicSchool" class="ctm-tab-top__title">各公立学校介绍</h5>
									<?php endif;?>
								</div>
							</div>
...
if (!empty($Sch['schoolvideo'])) {
    ?>
											<video class="video" controls="controls" src="<?php echo ($Sch['schoolvideo']); ?>" width="100%" id="promotionVid" loading="lazy">
												<!-- autoplay="autoplay" -->
												<?php
} else {
    ;
...
?>
								...
								<div class="col-md-8" id="bodyCList" style="<?php echo $displayCList; ?>" loading="lazy">
									<div class="blog-title-single">
										...
										<!-- ==========  Title  ========== -->
										<h4 class="title-1 bold ctm-widget-title">课程列表</h4>
...
?>
															...
															<div class="animated fadeIn ctm-btnFilter <?php echo $filterActiveAll__show; ?>"
																onclick="location.href='school-info.php?id=<?php echo $id; ?>&fid=0&tid=<?php echo $tid; ?>'">
																<span>全部</span></div>
...
while ($row = $dosql->GetArray()) {
    ...
    ?>
															<div class="animated fadeIn ctm-btnFilter <?php echo $filterActive__show; ?>"
																onclick="location.href='school-info.php?id=<?php echo $id; ?>&fid=<?php echo $row['bh']; ?>&tid=<?php echo $tid; ?>'">
																<span><?php echo $row['cname']; ?></span></div>
															<?php
}
?>
															<!-- </div> -->
...
?>
															...
															<div class="animated fadeIn ctm-btnFilter <?php echo $filterActiveAll__showCType; ?>"
																onclick="location.href='school-info.php?id=<?php echo $id; ?>&fid=<?php echo $fid; ?>&tid=0'">
																<span>全部</span></div>
...
foreach ($fieldsel as $value) {
    ...
    while ($vaC = $dosql->GetArray()) {
        ...
        if ($vaC['type'] == $vaT[1]) {
            ...
            $filter__onCType = getCType($vaC['type']);
            ?>

															<div class="animated fadeIn ctm-btnFilter <?php echo $filterActive__showCType; ?>"
																onclick="location.href='school-info.php?id=<?php echo $id; ?>&fid=<?php echo $fid; ?>&tid=<?php echo $vaC['type']; ?>'">
...
while ($row = $dosql->GetArray()) {
    ...
    ?>
												...
												<div class="gdlr-core-course-item-list ctm-pgSchool__itemList"
													...
													<h3 class="gdlr-core-course-item-title">
														<!-- <span class="gdlr-core-course-item-id gdlr-core-skin-caption" ></span> --><?php echo ($row['title']); ?></h3>
													<div class="gdlr-core-course-item-info-wrap">
														<div class="gdlr-core-course-item-info"><span class="gdlr-core-head">课程类别 : </span><span
																class="gdlr-core-tail"><?php echo (getCType($row['type'])); ?></span></div>
														<?php
    ...
    if (strpos($row['ks'], '-')) {
        ...
        $cDuration_yr = ' / 大约 ' . ceil($row['ks'] / 52) . ' 年';
    }
    ...
    if (!empty($row['kcfw'])) {
        ...
        if (!empty($cMajor['pid'])) {
            ...
            $faculty_cTitle = $faculty['cname'];
            $cMajor_title = getCnameByBh($row['kcfw']);
        } else {
            $faculty_cTitle = '暂无';
            $cMajor_title = '暂无';
        }
    ...
    ?>
														...
														<div class="gdlr-core-course-item-info"><span class="gdlr-core-head">学科 : </span><span
																class="gdlr-core-tail"><?php echo ($faculty_cTitle); ?></span></div>
														<div class="gdlr-core-course-item-info"><span class="gdlr-core-head">主修领域 : </span><span
																class="gdlr-core-tail"><?php echo ($cMajor_title); ?></span></div>
														<div class="gdlr-core-course-item-info"><span class="gdlr-core-head">课时 : </span><span
																class="gdlr-core-tail"><?php echo ($row['ks']); ?> 周<?php echo $cDuration_yr; ?></span></div>
														<div class="gdlr-core-course-item-info"><span class="gdlr-core-head">学费 : </span><span
																class="gdlr-core-tail"><?php echo ($fees_format); ?></span></div>
													</div>
													<a href="course-info.php?cid=<?php echo ($id); ?>&id=<?php echo ($row['id']); ?>"
...
?>
								...
								<div class="col-md-8" id="bodyHotel" style="display: none;">
									<div class="blog-title-single">
										...
										<h4 class="title-1 bold ctm-widget-title">酒店管理学院</h4>
								...
								<div class="col-md-8" id="bodyEng" style="display: none;">
									<div class="blog-title-single">
										...
										<h4 class="title-1 bold ctm-widget-title">英语课程</h4>

										<h6>英语课程简介</h6>
										<p>TAFE西悉尼（TAFE Western Sydney Institute）是一家致力于提供高品质语言课程的职业教育和高等教育培训机构。TAFE西悉尼拥有120年的历史，并一直以为客户提供创新的服务和灵活的课程而自豪。</p>
										<p>
											西悉尼（WSI）拥有10个校区，网上和远程教育中心以及一个坐落于西悉尼地区的专业培训中心，并一直为学生提供优秀的教师和良好的学习环境。西悉尼为学生提供各种英语课程培训和服务，包括为海外学生提供的英语强化课程（English Language Intensive Courses
											for Overseas
											Students）。西悉尼的英语强化课程专门为学生提供本科学习或研究生入学前对英文要求的专业培训。在西悉尼学校学习语言课程的同时还可以和当地学生一起感受到多文化之间的碰撞和交流，不仅有澳大利亚的学生，更有很多来自不同国家文化的国际留学生。此英语课程是通往西悉尼大学以及多所澳大利亚其他大学本科学位，文凭已经多项证书课程的捷径。
										</p>
								...
								<div class="col-md-8" id="bodyCollege" style="display: none;">
									<div class="blog-title-single">
										...
										<h4 class="title-1 bold ctm-widget-title">预科/文凭课程</h4>
								...
								<div class="col-md-8" id="bodyNews" style="display: none;">
									<div class="blog-title-single">
										...
										<h4 class="title-1 bold ctm-widget-title">新闻</h4>
...
?>
							...
							<!-- <div class="animated fadeIn row row-otherSchool" style="margin-bottom: 5%; transform: scale(0.9); <?php echo $displayOverview; ?>">
			                	<h4 class="title-1 bold ctm-widget-title">推荐院校</h4> -->
							<!-- </div> -->
