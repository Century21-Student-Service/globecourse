...
function getLevels()
{
    ...
    if ($state_id) {
        $sql = "SELECT `id`,IF(name_en IS NULL OR name_en = '', `name`, name_en) AS `name` FROM `level` WHERE 1 = 1 $regional_sql AND id IN
        ...
        $sql = "SELECT `id`,IF(name_en IS NULL OR name_en = '', `name`, name_en) AS `name` FROM `level` WHERE 1 = 1 $regional_sql ORDER BY id;";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
    }
...
function getStates()
{
    ...
    if (isset($regional)) {
        ...
        $sql = "SELECT `id`,IF(name_en IS NULL OR name_en = '', `name`, name_en) AS `name`,code FROM `state` ORDER BY id";
    if ($country_id){
        	$sql = "SELECT `id`,IF(name_en IS NULL OR name_en = '', `name`, name_en) AS `name`,code FROM `state` where country_id = '{$country_id}' ORDER BY id";
        }
        $stmt = $conn->prepare($sql);
        $stmt->execute();
    }
...
function getCountry(){
	global $conn;
	$sql = "SELECT `id`,IF(name_en IS NULL OR name_en = '', `name`, name_en) AS `name` FROM `country` ORDER BY id";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    echo json_encode($stmt->fetchAll(PDO::FETCH_ASSOC), JSON_UNESCAPED_UNICODE);
}
...
function getFields($return = false)
{
    ...
    $sql_child = "SELECT `id`,IF(name_en IS NULL OR name_en = '', `name`, name_en) AS `name`,`p_id` FROM field WHERE 1 = 1 ";
    if (strlen($regional)) {
        if ($regional) {
    ...
    $sql_parent = "SELECT `id`,IF(name_en IS NULL OR name_en = '', `name`, name_en) AS `name` FROM field WHERE deep = 0 AND id IN(" . implode(',', $pids) . ");";
    // echo $sql_parent;die;
