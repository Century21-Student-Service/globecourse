...
<script type="text/javascript" src="./lib/ueditor/1.4.3/lang/zh-cn/zh-cn.js"></script>
  <script>
    ...
    $(function () {
      ...
      if (id) {
          $.get(
            "util/course_hotOperation?op=4&id=" + id,
            res => {
              res = JSON.parse(res);
      ...
      $("#btn_save").click(e => {
        ...
        $.post('util/course_hotOperation.php?op=7', JSON.stringify(data_new), res => {
    ...
    function uploadFile(file, callback, progressId, video = false) {
      ...
      const url = video ? 'util/course_hotOperation.php?op=9' : 'util/course_hotOperation.php?op=5';
      $.ajax({
        url,
        data: formData,
        processData: false,
    ...
    function deletepic(pic_path) {
      if (!Array.isArray(pic_path))
        pic_path = [pic_path]
      $.post('util/course_hotOperation.php?op=6', {
