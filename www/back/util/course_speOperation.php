...
function getList()
{
    ...
    $sql_detail = "SELECT  id, title,title_en, tag,tag_en, image, sort, add_user, FROM_UNIXTIME(update_time) as update_time, read_count
                    ...
                    LIMIT $offset, $limit ;";
    $sql = "SELECT id FROM course_spe i ";
...
{
    global $conn;
    $sql_delete = "DELETE FROM course_spe WHERE id=:id;";
...
function saveData()
{
	...
	if (!empty($data['id'])) {
        //更新现有数据
        $sql = "UPDATE course_spe SET update_time = ".time().",";
        ...
        $sql = "INSERT INTO course_spe (`title`,`title_en`,`tag`,`tag_en`, `image`,`sort`,`content`,`content_en`,`add_user`,`add_time`,`update_time`) 
